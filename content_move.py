...
iv = ''
region = ''
api_url = 'https://api.pkappnew.buzz'
...
for task in tasks:
    ...
    in_token = MyApi.login(api_url, task['in_info']['account'],
                           task['in_info']['password'], enKey)
    if in_token == '' or in_token == '-1':
        continue

    # 订阅组获取
    pay_group_ids = MyApi.myGroupList(api_url, in_token,
                                      task['in_info']['account'])
    if pay_group_ids == []:
        continue

    for out_user in task['out_info']['infos']:
        out_token = MyApi.login(api_url, out_user['account'],
                                out_user['password'], enKey)
        ...
        while page_number_index <= page_number:
            ...
            out_contents.extend(
                MyApi.getContentList(api_url, out_token, page_number_index + 1,
                                     out_contents_page_size))
            page_number_index += 1
        ...
        for out_content in out_contents:
            ...
            publish_data['payPermissionType'] = out_content['works'][
                'payPermission']['type']
            publish_data['payGroupId'] = pay_group_ids[0]['groupId']
            publish_data['payPrice'] = out_content['works']['payPermission'][
                'price']
            publish_data['limitFreeDays'] = out_content['works'][
                'payPermission']['limitFreeDays']
            publish_data['replyPermissionType'] = out_content['works'][
                'replyPermission']['type']

            in_publish = MyApi.publish(api_url, in_token, publish_data,
                                       task['in_info']['account'])

            out_delete = MyApi.contentDelForServer(server_url, server_token,
                                                   str(out_content['wid']))
